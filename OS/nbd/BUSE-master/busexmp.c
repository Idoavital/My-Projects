...
static int xmp_read(struct buse_req* requset_st, void *userdata)
{
  if (*(int *)userdata)
    fprintf(stderr, "R - %lu, %u\n", requset_st->offset, requset_st->len);
  memcpy(requset_st->data, (char *)data + requset_st->offset, requset_st->len);

  buseDone(requset_st ,READ);
  return 0;
}

static int xmp_write(struct buse_req* requset_st, void *userdata)
{
  if (*(int *)userdata)
    fprintf(stderr, "W - %lu, %u\n", requset_st->offset, requset_st->len);

  memcpy((char *)data + requset_st->offset, requset_st->data, requset_st->len);
  buseDone(requset_st ,WRITE);
  return 0;
}
