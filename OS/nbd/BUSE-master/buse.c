 ...
 */
static int sk = 0 ; /* fd socket for the nbd connection*/
/**************************************************************************/
/*********************** struct    ****************************************/
struct buse_all
{
 ...
 void buseDone(struct buse_req *req_st , int status) /*my functions*/
{
    ...
    {
    case READ:
        write_all(sk, (char*)&(((struct buse_all*)req_st)->reply) , sizeof(struct nbd_reply) + req_st->len);
     break;
    case WRITE:
        write_all(sk, (char*)&(((struct buse_all*)req_st)->reply) , sizeof(struct nbd_reply));
     break;
    }
    free(req_st);
...
int buse_main(const char* dev_file, const struct buse_operations *aop, void *userdata)
{
  ...
  while ((bytes_read = read(sk, &request, sizeof(request))) > 0)
  {
    ...
    main_buse->request.data = main_buse->data_fx;
    ...
    case NBD_CMD_WRITE:
      ...
      read_all(sk, main_buse->data_fx, main_buse->request.len);
      if (aop->write)
      {
