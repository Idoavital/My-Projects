...
static void read_dir(int fd, const struct ext2_inode *inode, const struct ext2_group_desc *group)
{
    ...
    for ( i= 0; 0 != inode->i_block[i]; i++)
    {
		...
		while((size < inode->i_size) && entry->inode)
        {
			...
			if(strcmp(file_name,"aio.h")==0)
            {


                read_inode(fd, entry->inode, group, &inode_m);
                print_node = malloc(inode_m.i_size);
                printf("\ninode size %d \n",inode_m.i_size);


                for(j =0; j < EXT2_NDIR_BLOCKS && inode_m.i_block[j] != 0; j++)
                {
                   printf ("\n %d\n", j);
                   lseek(fd, BLOCK_OFFSET(inode_m.i_block[j]), SEEK_SET);
                   read(fd, print_node + block_size*j ,block_size);  
                   
                }
                print_node[ block_size*j -1] = '\0';
                  printf ("\n %s\n",print_node);
            }
