...
pQueue<T,Container,Cmp >::pQueue( int max_free_token , int num_of_token ): m_counter(0),
                m_num_block (num_of_token - max_free_token),
                m_num_token(num_of_token),
                m_flag_run(true)
...
/*****************************************************************************/
template <typename T ,typename  Container ,typename Cmp >
void pQueue<T,Container,Cmp >::pop(T& item)
  {
    std::unique_lock<std::mutex> mlock(mutex_);
    m_counter++; 
...
/*****************************************************************************/
template <typename T ,typename  Container ,typename Cmp >
void pQueue<T,Container,Cmp >::push(const T& item)
  {
    std::unique_lock<std::mutex> mlock(mutex_);
    queue_.push(item);
    mlock.unlock();
...
/*****************************************************************************/
template <typename T ,typename  Container ,typename Cmp >
void pQueue<T,Container,Cmp >::push(T&& item)
  {
    std::unique_lock<std::mutex> mlock(mutex_);
